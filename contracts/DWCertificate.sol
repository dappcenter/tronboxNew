...
contract Utils{
function strConcat(string calldata s1, string calldata s2) pure external returns(string memory);
function uintToString(uint256 v) pure external returns (string memory);
}
contract ERC721 {
// Events
...
function setUtilAddr(address _addr) onlyCLevel external{
utilAddr = _addr;
}
...
Utils u = Utils(utilAddr);
return u.strConcat(baseTokenUri, u.uintToString(_tokenId));
}
