...
function certificateIncre(uint256 _recordId) external;
function certificateDecre(uint256 _recordId) external;
function bindCertificate(uint256 _recordId, uint256 _certId)  public;
}
...
contract WRCertificate2 is ERC721, WRPausable{
struct Certificate{
...
event Award(address indexed _owner, string indexed _challengeName, uint256 _certId, uint256 _recordId, uint256 _challengeTime, string _challengeLocation, string _videoUri, uint256 _checkTime, string _remark, uint256 _status);
event CheckResult(uint256 indexed _certId,bool _isAgree);
event Burned(address indexed _owner, uint256 _tokenId);
event ReturnBack(address indexed _owner, uint256 _tokenId);
...
function getCertificateInfo(uint256 _tokenId) view public returns(bool _isRecordHolder, string memory _challengeName, uint256 _recordId, uint256 _challengeTime, string memory _challengeLocation, string memory _videoUri, uint256 _checkTime, string memory _remark, uint256 _status){
...
SimpleRecord(recordAddr).certificateIncre(certificates[_certId].recordId);
}else{
certificates[_certId].status = 2;
_burn(certIndexToOwner[_certId], _certId);
}
certificates[_certId].checkTime = now;
emit CheckResult(_certId, _isAgree);
}

function award(address _owner, string memory _challengeName, uint256 _recordId, uint256 _challengeTime, string memory _challengeLocation, string memory _videoUri, string memory _remark) onlyOperator public returns(uint256 _certId){
...
SimpleRecord(recordAddr).bindCertificate(_recordId, _certId);
emit Award(_owner, _challengeName, _certId, _recordId, _challengeTime, _challengeLocation, _videoUri, 0, _remark, 0);
...
SimpleRecord(recordAddr).certificateDecre(certificates[_tokenId].recordId);
}
certificates[_tokenId].status = 4;
_burn(certIndexToOwner[_tokenId], _tokenId);
}
