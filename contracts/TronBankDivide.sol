...
contract SimpleStake{
    ...
    function updateHolderLimit(uint256 _index, uint256 _count, bool _active) public;
    function resetHolderLimit(address _addr) public;
}
...
contract TronBankDivide is Pausable{
    ...
    uint256 overLimitStake;
    uint256 unfreezingStake;
    uint256 divide;
    bool active;
    uint256 divideUnit;
    uint256 public divideNum = 0;//分红次数
    uint256 public convertUnit = 1e2;
    ...
    function setDivideNum(uint256 value) onlyOperator public{
        divideNum = value;
    }
    ...
    function addPlan10(uint256 _trc10Id, address _tokenPoolAddr) onlyOperator public{
        divide10PlanArray.push(Objects.Divide10Plan({isExpired : false, trc10Id : _trc10Id, totalProfit : 0, divide : 0, active : true, tokenPoolAddr : _tokenPoolAddr, divideUnit : 0}));
    }

    function addPlan20(address _trc20Addr, address _tokenPoolAddr) onlyOperator public{
        divide20PlanArray.push(Objects.Divide20Plan({isExpired : false, trc20Addr : _trc20Addr, totalProfit : 0, divide : 0, active : true, tokenPoolAddr : _tokenPoolAddr, divideUnit : 0}));
    }

    function withdrawalAll() whenNoPaused public{
        SimpleStake s = SimpleStake(stakeAddr);
        s.resetHolderLimit(msg.sender);
    ...
    function getCurrentDivideTrxInfo() view public returns(uint256 _divideNum, uint256 _maxIndex, uint256 _currentIndex,
        uint256 _totalStake, uint256 _totalProfit,uint256 _effectiveStake, uint256 _overLimitStake, uint256 _unfreezingStake, bool _active){
        ...
        if(paused){
            ...
            _effectiveStake = effectiveStake;
            ...
            (_maxIndex, _totalStake, _effectiveStake, _overLimitStake, _unfreezingStake) = s.getStakeInfo();
            _currentIndex = 0;
    ...
    function createDivide() onlyOperator whenNoPaused public returns(bool){
        divideNum++;
        SimpleStake s = SimpleStake(stakeAddr);
        (maxIndex, totalStake, effectiveStake, overLimitStake, unfreezingStake) = s.getStakeInfo();
        require(effectiveStake > 0,"effectiveStake is zero");
        if(poolAddr != address(0)){
            SimplePool p = SimplePool(poolAddr);
            (totalProfit, active) = p.withdrawalProfit();
            if(active){
                divideUnit = totalProfit.mul(convertUnit).div(effectiveStake);
            }else{
                divideUnit = 0;
            }
        }else{
        ...
        for(uint256 k = 0 ; k < divide10PlanArray.length; k++){
            if(!divide10PlanArray[k].isExpired){
                ...
                if(divide10PlanArray[k].active){
                    isHasProfit10 = true;
                    divide10PlanArray[k].divideUnit = divide10PlanArray[k].totalProfit.mul(convertUnit).div(effectiveStake);
                }else{
                    divide10PlanArray[k].divideUnit = 0;
                }
                divide10PlanArray[k].divide = 0;
            }
        }
        for(uint256 q = 0 ; q < divide20PlanArray.length; q++){
            if(!divide20PlanArray[q].isExpired){
                ...
                if(divide20PlanArray[q].active){
                    isHasProfit20 = true;
                    divide20PlanArray[q].divideUnit = divide20PlanArray[q].totalProfit.mul(convertUnit).div(effectiveStake);
                }else{
                    divide20PlanArray[q].divideUnit = 0;
                }
                divide20PlanArray[q].divide = 0;
            }
        }
        currentIndex = 0;
    ...
    function doNextPay() onlyOperator whenPaused public returns(bool isSuccess, uint256 _currentIndex){
        ...
        if(currentIndex >= maxIndex){
            (currentAddr, stakeValue, limit) = s.getTokenStakeByIndex(0);
            if(active && totalProfit > divide){
                trxBalance[currentAddr] = trxBalance[currentAddr].add(totalProfit.sub(divide));
            }
            for(uint256 m = 0 ; m < divide10PlanArray.length; m++){
                if(!divide10PlanArray[m].isExpired && divide10PlanArray[m].active){
                    if(divide10PlanArray[m].totalProfit > divide10PlanArray[m].divide){
                        divideValue = divide10PlanArray[m].totalProfit.sub(divide10PlanArray[m].divide);
                        divide10PlanArray[m].tokenBalance[currentAddr] = divide10PlanArray[m].tokenBalance[currentAddr].add(divideValue);
                    }
                }
            }
            for(uint256 n = 0 ; n < divide20PlanArray.length; n++){
                if(!divide20PlanArray[n].isExpired && divide20PlanArray[n].active){
                    if(divide20PlanArray[m].totalProfit > divide20PlanArray[m].divide){
                        divideValue = divide20PlanArray[m].totalProfit.sub(divide20PlanArray[m].divide);
                        divide20PlanArray[m].tokenBalance[currentAddr] = divide20PlanArray[m].tokenBalance[currentAddr].add(divideValue);
                    }
                }
            }
            paused = false;
            s.doUnPause();
            emit DivideComplete(divideNum,maxIndex, effectiveStake, totalProfit);
        }else{
            (currentAddr, stakeValue, limit) = s.getTokenStakeByIndex(currentIndex);
            if(limit > 0){
                if(active){
                    divideValue = getDivideValue(divideUnit, stakeValue);
                    trxBalance[currentAddr] = trxBalance[currentAddr].add(divideValue);
                    divide = divide.add(divideValue);
                    emit DivideStep(divideNum, maxIndex, currentIndex, currentAddr,
                        divideValue, stakeValue, effectiveStake, totalProfit);
                }
                for(uint256 k = 0 ; k < divide10PlanArray.length; k++){
                    if(!divide10PlanArray[k].isExpired && divide10PlanArray[k].active){
                        ...
                        divide10PlanArray[k].divide = divide10PlanArray[k].divide.add(divideValue);
                        emit DivideStep10(divideNum, maxIndex, currentIndex, currentAddr, divide10PlanArray[k].trc10Id,
                            divideValue, stakeValue, effectiveStake, divide10PlanArray[k].totalProfit);
                    }
                }
                for(uint256 q = 0 ; q < divide20PlanArray.length; q++){
                    if(!divide20PlanArray[q].isExpired && divide20PlanArray[q].active){
                        ...
                        divide20PlanArray[q].divide = divide20PlanArray[q].divide.add(divideValue);
                        emit DivideStep20(divideNum, maxIndex, currentIndex, currentAddr, divide20PlanArray[q].trc20Addr,
                            divideValue, stakeValue, effectiveStake, divide20PlanArray[q].totalProfit);
                    }
                }
                s.updateHolderLimit(currentIndex, 1, false);
            }
        }
        _currentIndex = currentIndex;
    }
